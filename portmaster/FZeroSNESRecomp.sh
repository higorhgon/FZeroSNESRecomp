#!/bin/bash
# PortMaster launcher for FZeroSNESRecomp.
#
# Target hardware: Anbernic H700-chip handhelds (RG34XX / RG34XX H / RG34XXSP),
# 720x480 physical panel, running muOS. See the "PortMaster / Anbernic H700
# (muOS)" section of the project README for how this port is built and
# packaged, and for what has and has not been verified on real hardware.
#
...
GAMEDIR="/$(dirname "$(readlink -f "$0")")/FZeroSNESRecomp"
cd "$GAMEDIR" || exit 1
...
# --- Video backend ---------------------------------------------------------
#
# The H700's Mali-G31 runs on Mesa's mainline Panfrost driver under muOS
# (not a Batocera-style vendor "mali" blob), so SDL's own driver probing
# should land on the correct backend (kmsdrm on a bare console session, or
# x11/wayland if muOS's PortMaster session happens to run one) without help.
...
# fzero-video.ini (Aspect/EnhancedRenderer/etc.) and config.ini (Fullscreen/
# WindowScale/etc.) are otherwise resolved next to the executable, but set
# this explicitly so it is unambiguous no matter how muOS invokes this
# script (symlink, wrapper, or a working directory PortMaster changed).
export FZERO_VIDEO_CONFIG="$GAMEDIR/fzero-video.ini"
...
# different, less-tested code path in src/sdl_main.c).
ROM=""
...
./FZeroSNESRecomp > "$GAMEDIR/log.txt" 2>&1
