...
This project statically recompiles *your own* F-Zero (USA) ROM into C code
at build time (`tools/regen.sh`). That ROM can never be committed to this
repo, uploaded as a GitHub Actions secret, or otherwise sent to GitHub's
infrastructure - Nintendo owns the copyright, and "just a secret" still
means storing the ROM on a third party's servers, which nothing here is
...
If you're also setting up the sibling `SuperMarioWorldRecomp` fork's
runner, you can reuse the same physical machine, but GitHub's per-repo
runner registration on a personal account means you'll register a second,
...
`/home/you/roms/fzero.sfc`. You'll pass this exact path as the `rom_path`
...
first run to take a while (QEMU-emulated compilation plus a from-source
SDL3 build is not fast), later runs will be quicker if Docker's layer/build
caches survive between runs on that machine.
